...
- **The emulator will not boot a game.** PCSXROO must be a Release build; a Devel build
  cannot boot at all. `tools\pcsxroo\build.cmd` defaults to Release.
- **Fresh install behaves oddly.** Run `tools\pcsxroo\seed-portable.ps1`. Without a BIOS the
  emulator stops at its setup wizard, before the server ever starts.

`tools\pcsxroo\smoke-test.ps1 -Bios` runs 28 checks over the whole path and is the quickest
