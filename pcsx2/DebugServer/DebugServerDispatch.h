...
namespace DebugServerDispatch
{
	...
	// would hang the calling client forever. Returns false on timeout, in which case fn may
	// still run afterwards - so fn must capture by value only, and must not reference
	// anything owned by the caller's stack.
	//
